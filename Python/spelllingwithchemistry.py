...
for i in range(len(periodicTable)/5):
    dict[periodicTable[i*5+1].lower()] = periodicTable[i*5].lower()

input = ['genius','functions','bacon','poison','sickness','ticklish']

def chemistry_spelling(input):
    output1 = ''
    output2 = []
    i = 0
    while i < len(input):
        if input[i:i+2] in dict and i+2 < len(input):
            output1 += input[i].upper() + input[i+1].lower()
            output2.append(dict[input[i:i+2]])
            i += 1
        elif input[i] in dict:
            output1 += input[i].upper()
            output2.append(dict[input[i]])
        i += 1
    output1 += ' ('
    for i in output2:
        output1 += i.lower() + ', '
